...
const PoolCardLP = ({ pool, userDataLoaded }) => {
	...
	return (
		<FCard>
			<div className="bsc-farm-item-header align-items-center">
			...
			{isView && (
				<div className="bsc-farm-item-more">
					...
					<div className="d-flex justify-content-between align-items-center">
						<span className="text-rest total-raise">Total Liquidity</span>
					...
					<>
						{pool.typeLP === 1 ? (
							<Button
								className="bsc-farm-item-more-bottom pd-top button-info"
								text={`Get ${pool.stakingToken.symbol}`}
								link
								click={() => {
									window.open(`/`, '_blank')
								}}
